...
def draw_menu(context, menu_name):
    menu = get_object_or_404(Menus, name=menu_name, parent=None)
    local_context = {'menu_item': menu}
    requested_url = context['request'].path
    try:
        active_menu_item = Menus.objects.get(explicit_url=requested_url)
...
def draw_menu_item_children(context, menu_item_id):
    menu_item = get_object_or_404(Menus, pk=menu_item_id)
